...
const Search = ({ videos} : {videos: Video[]}) => {
    ...
    return ( 
        ...
        <div className='flex gap-10 mb-10 border-b-2 border-gray-700 md:fixed z-50 bg-primary w-full'>
            <p onClick={() => setIsAccounts(true)} className={`text-xl text-white font-semibold cursor-pointer ${accounts} mt-2`}>
            Accounts
            </p>
            <p className={`text-xl font-semibold  text-white cursor-pointer ${isVideos} mt-2`} onClick={() => setIsAccounts(false)}>
            Posts
            </p>
        </div>
        {isAccounts ? (
            ...
            {searchedAccounts.length > 0 ? (
                ...
                <Link key={idx} href={`/profile/${user._id}`}>
                    <div className=' flex gap-3 p-2 cursor-pointer font-semibold rounded border-b-2 border-gray-700'>
                    <div>
                        <Image width={50} height={50} className='rounded-full' alt='user-profile' src={user.image}/>
                    </div>
                    <div>
                        <div>
                        <p className='flex gap-1 items-center text-lg font-bold text-white'>
